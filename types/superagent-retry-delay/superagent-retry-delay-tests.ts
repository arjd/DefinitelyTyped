...
import * as Request from 'superagent-retry-delay';
...
request
    .get('/some-url')
    .retry(3, [1000, 5000, 10000])
    .end((err, res) => {
        // Do something
    });

// retry 3 times with increasing delays and stop retry loop if 401 or 404
request
    .get('/some-url')
    .retry(3, [1000, 5000, 10000], [401, 404])
    .end((err, res) => {
        // Do something
    });

// passes all superagent tests (excludes retry with callback)
request
    .post('/api/pet')
...
superagent.serialize['application/xml'] = (obj: any) => 'generated xml here';

// Parsing response bodies
request('/search')
    .end((res: superagent.Response) => {
...
// Authentication
superagent.get('http://tobi:learnboost@local').end(callback);
...
(() => {
    ...
    const req = superagent.get('/some.json');
    req.pipe(stream);
})();
...
request
    .post('/upload')
    .attach('image', 'path/to/tobi.png')
    .on('progress', (progress: superagent.ProgressEvent) => {
...
// adapted from: https://github.com/visionmedia/superagent/blob/v2.0.0/test/client/superagent.js#L110
request
    .get('/blob')
    .responseType('blob')
    .end((err, res) => {
...
const echoPlugin = (request: superagent.SuperAgentRequest) => {
    req.url = '' + req.url;
